...
impl Mover{
    ...
    fn attract(&self, m:&Mover) -> Vec2{
        ...
        let strength = -1.0 * ( self.mass * m.mass) / (d * d); 
        force * strength
    }
...
fn model(app: &App) -> Model {
    ...
    Model {
        movers: v,
        attractor: Attractor::new(vec2(0.0,0.0), 20.0)
    }
}
...
fn update(app: &App, model: &mut Model, _update: Update) {
    for i in 0..model.movers.len() {
        for j in 0..model.movers.len() {
            if i != j {
                let force = model.movers[j].attract(&model.movers[i]);
    ...
    // for (i, mover ) in &mut model.movers{
    //     let f = model.attractor.attract(mover);
    //     mover.apply_force(f);
    //     mover.update();
