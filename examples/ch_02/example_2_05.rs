...
impl Mover{
    fn new(rect: Rect, mass: f32) -> Self{
    ...
    fn apply_drag(&mut self, area: &FrictionArea){
        ...
        let dragnitude = area.friction_coefficient * speed * speed;
...
fn model(app: &App) -> Model {
    ...
    for _ in 0..10 {
        ...
        let m: f32 = random_range(0.5, 4.0);
        v.push(Mover::new(Rect::from_xy_wh(vec2(x,y), vec2(32.0, 32.0)), m));
    }


    Model {
        movers: v,
        friction_area: FrictionArea::new(Rect::from_corners(rect.mid_left(), rect.bottom_right()), 0.1)
    }
}
